...
char *intersect ( int size, char *arg1, char *arg2, char *arg3 )
	{
		int j, validity;
		for ( j = 0; j < size; j++ )
			{
				// cubeIntersect_2 has the value of LOGIC AND between arguments' bits // 
				if ( (strncmp ( &arg1 [j], "1", 1) == 0) && (strncmp ( &arg2 [j], "1", 1) == 0) ) 
						strncpy ( &arg3 [j], "1", 1);
				else
						strncpy ( &arg3 [j], "0", 1);
			}
...
Tcl_ObjCmdProc *do_cube_intersect_2 ( ClientData clientData, Tcl_Interp *interp, int objc, Tcl_Obj *CONST objv[] )
  {
		...
		cubeIntersect_2 = intersect ( size, arguments [0], arguments [1], cubeIntersect_2 );
...
void *do_cube_cover_2 ( ClientData clientData, Tcl_Interp *interp, int objc, Tcl_Obj *CONST objv[] )
  {
		...
		cover = cube_cover ( size, arguments [0], arguments [1] );
...
int cube_cover ( int size, char *arg1, char *arg2 )
	{
		int j;
		for ( j = 0; j < size; j++ )
			{
				/************************************************************
				 * strncmp return 1, 0, -1 if 1st argument is greater than, *
				 * equal to or less than 2nd argument, respetively.         *
				 ************************************************************/ 
				if ( (strncmp ( &arg1 [j], &arg2 [j], 1) < 0) ) // for each bit argument0 < argument1
					{
						if ( sharpIsActive == 1)
